...
if python3 - "$DOCKER_WF" "$PROMOTE_WF" "$BACKEND_CI" "$SYNC_WF" <<'PY'
import re, sys
...
if "publish" in jobs:
    ...
    if "existing-publisher-run.json" not in p or "run_attempt" not in p or '"status": "completed"' not in p or '"conclusion": "success"' not in p:
        errors.append("existing debug-sha recovery must verify the publisher run through the Actions API")
