...
# Pure shell + Python stdlib contracts (no dependency installation).
...
SYNC_WF="${ROOT_DIR}/.github/workflows/sync-upstream-main.yml"
BACKEND_CI="${ROOT_DIR}/.github/workflows/backend-ci.yml"
PROVENANCE_SCRIPT="${ROOT_DIR}/deploy/verify-image-provenance.py"
CI_IMAGE_SCRIPT="${ROOT_DIR}/deploy/verify-ci-image.py"
CI_EVIDENCE_SCRIPT="${ROOT_DIR}/deploy/ci_evidence.py"
FAIL=0
...
need_file "$SYNC_WF" || true
need_file "$BACKEND_CI" || true
need_file "$PROVENANCE_SCRIPT" || true
need_file "$CI_IMAGE_SCRIPT" || true
need_file "$CI_EVIDENCE_SCRIPT" || true

if python3 -c 'import yaml' >/dev/null 2>&1; then
  if python3 - "$DOCKER_WF" "$PROMOTE_WF" "$SYNC_WF" "$BACKEND_CI" <<'PY'
import sys, yaml
for path in sys.argv[1:]:
    with open(path, encoding="utf-8") as f:
        parsed = yaml.safe_load(f)
    if not isinstance(parsed, dict) or "jobs" not in parsed:
        raise SystemExit(f"{path}: workflow must parse to a mapping with jobs")
PY
  then
    pass "workflow YAML parses with PyYAML"
  else
    fail "workflow YAML parsing failed"
  fi
elif command -v ruby >/dev/null 2>&1; then
  if ruby -e 'require "yaml"; ARGV.each { |path| doc = YAML.load_file(path); raise "#{path}: missing jobs" unless doc.is_a?(Hash) && (doc.key?("jobs") || doc.key?(:jobs)) }' \
    "$DOCKER_WF" "$PROMOTE_WF" "$SYNC_WF" "$BACKEND_CI"; then
    pass "workflow YAML parses with Ruby stdlib"
  else
    fail "workflow YAML parsing failed"
  fi
else
  fail "no YAML parser available for workflow structural validation"
fi
...
file_contains "$BACKEND_CI" 'verify-ci-image-test\.py' "backend-ci runs primary/standby image evidence tests"
...
file_contains "$BACKEND_CI" '^  test:' "backend-ci preserves the historical test job"
file_contains "$BACKEND_CI" 'docker-compose-security-test\.sh' "backend-ci retains compose security tests"
file_contains "$BACKEND_CI" 'docker-runtime-resources-test\.sh' "backend-ci retains runtime resource tests"
file_contains "$BACKEND_CI" 'test-caddyfile-cache\.sh' "backend-ci retains Caddy cache tests"
file_contains "$SYNC_WF" 'merge-base --is-ancestor origin/main upstream/main' "main sync refuses divergent fork history"
file_contains "$SYNC_WF" 'git merge --ff-only upstream/main' "main sync is fast-forward only"
file_contains "$SYNC_WF" 'force-with-lease=refs/heads/main:' "main sync binds its push lease"
if grep -Eq 'git (reset --hard|push .*--force([^ -]|$))' "$SYNC_WF"; then
  fail "main sync must not reset or unconditionally force-push"
else
  pass "main sync avoids destructive reset and unconditional force-push"
fi

if python3 - "$DOCKER_WF" "$PROMOTE_WF" "$BACKEND_CI" "$SYNC_WF" "$CI_IMAGE_SCRIPT" "$CI_EVIDENCE_SCRIPT" <<'PY'
import re, sys
...
sync = load(sys.argv[4])
ci_image = load(sys.argv[5])
ci_evidence = load(sys.argv[6])
...
for required in ("test-unit", "test-integration", "test"):
...
if "test-unit" in backend_jobs and "test-integration" in backend_jobs:
    ...
    for name, block in (("test-unit", unit), ("test-integration", integration)):
        ...
        if not re.search(r"actions/setup-go@[0-9a-f]{40}\s+# v6", block) or "go-version-file: backend/go.mod" not in block:
            errors.append(f"backend-ci {name} must use the commit-pinned Go toolchain setup")
...
if "test" in backend_jobs:
    test_job = backend_jobs["test"]
    if not re.search(r"(?m)^    if:\s*always\(\)\s*$", test_job):
        errors.append("backend-ci test aggregator must run with if: always()")
    if "needs: [test-unit, test-integration]" not in test_job:
        errors.append("backend-ci test aggregator must depend on both parallel suites")
    if not re.search(r'test\s+"\$\{UNIT_RESULT\}"\s+=\s+success', test_job):
        errors.append("backend-ci test aggregator must require test-unit success")
    if not re.search(r'test\s+"\$\{INTEGRATION_RESULT\}"\s+=\s+success', test_job):
        errors.append("backend-ci test aggregator must require test-integration success")
    if "UNIT_RESULT" not in test_job or "INTEGRATION_RESULT" not in test_job:
        errors.append("backend-ci test aggregator must fail unless both suites succeed")

for workflow_name, workflow in (("docker", docker), ("promote", promote), ("backend", backend), ("sync", sync)):
    for match in re.finditer(r"(?m)^\s*(?:-\s*)?uses:\s*([^\s#]+)", workflow):
        ...
        if not re.fullmatch(r"[0-9a-f]{40}", ref):
            errors.append(f"{workflow_name} action is not pinned to a full commit SHA: {action}")
...
if "publish" in jobs:
    ...
    if "verify-ci-image.py publisher" not in p or '"status": "completed"' not in ci_image or '"conclusion": "success"' not in ci_image:
        errors.append("existing debug-sha recovery must verify the original primary/standby publisher")
    ...
    if "ref: ${{ github.sha }}" not in jobs.get("build", "") or "ref: ${{ github.sha }}" not in p:
        errors.append("docker build and publish checkouts must use the event SHA")
...
if "promote" not in job_blocks(promote):
    ...
    if "ref: ${{ github.sha }}" not in p:
        errors.append("promote checkout must use the immutable workflow event SHA")
    if 'EXPECTED_REVISION}" != "${EVENT_SHA}' not in p:
        errors.append("promote must require expected_revision to equal github.sha")
    if "origin/mine" not in p or "origin/debug" not in p:
        errors.append("promote must check origin/mine and origin/debug")
    if p.count('repos/${CI_PRIMARY_REPOSITORY}/git/ref/heads/mine') < 3 or p.count('repos/${CI_PRIMARY_REPOSITORY}/git/ref/heads/debug') < 3:
        errors.append("promote must check heads early, before writes, and immediately before floating mine")
    if "verify-ci-image.py debug" not in p or '--run-id "$SOURCE_RUN_ID"' not in p or '--sha "$FULL_SHA" --digest "$SOURCE_DIGEST"' not in p:
        errors.append("promote must bind source metadata to exact CI run, revision and digest")
    if 'WORKFLOW = ".github/workflows/docker-branch.yml"' not in ci_image or 'REF = "refs/heads/debug"' not in ci_image:
        errors.append("source evidence verifier must fix the debug workflow and branch")
    if "head_branch" not in ci_evidence or "head_sha" not in ci_evidence or 'run.get("event") in ("push", "workflow_dispatch")' not in ci_evidence:
        errors.append("CI evidence must bind source run identity and allowed event")
    if "imagetools create" not in p:
...
promote_top = promote.split("jobs:", 1)[0]
if re.search(r"(?m)^permissions:[\s\S]*packages:\s*write", promote_top):
    errors.append("promote top-level must not set packages: write (only promote job)")
...
if "len(matches) == 1" not in ci_evidence:
    errors.append("promote must require exactly one debug-image-metadata artifact")
if "expired" not in ci_evidence:
    errors.append("promote must reject expired debug-image-metadata")
if "Evidence list exceeds pagination limit" not in ci_evidence or 'github.pages(f"repos/{repository}/actions/runs/{run_id}/artifacts"' not in ci_evidence:
    errors.append("promote must enumerate all artifacts and fail when uniqueness cannot be proven")
...
for name, workflow in (("docker", docker), ("promote", promote)):
    for marker in ("CI_FALLBACK_CONTRACT: '1'", "vars.CI_PRIMARY_REPOSITORY", "vars.CI_SOURCE_REF", "secrets.CI_PUBLISH_TOKEN || secrets.GITHUB_TOKEN"):
        if marker not in workflow:
            errors.append(f"{name} must preserve the standby contract: {marker}")
    if '"source_run_id": os.environ.get("GITHUB_RUN_ID")' in workflow:
        errors.append(f"{name} must distinguish the primary source run from the actual builder")
if '--repository "$PUBLISHER_REPOSITORY"' not in docker or '--repository "$PUBLISHER_REPOSITORY"' not in promote:
    errors.append("image provenance must verify the actual publisher repository")
if '--ref "$PUBLISHER_REF"' not in docker or '--ref "$PUBLISHER_REF"' not in promote:
    errors.append("image provenance must verify the actual publisher ref")
