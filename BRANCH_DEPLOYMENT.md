...
The server's `gci`/`gh` compatibility entry can replay a billing-blocked build
on its private standby account. The workflow keeps the primary image address,
source branch and source run ID, while `publisher_repository`, `publisher_ref`,
and the publisher run/attempt identify the actual builder. SLSA provenance is
checked against that actual builder, including its standby ref. The read-only
`deploy/verify-ci-image.py` consumer verifies primary artifacts or the preserved
`ci-fallback-receipt.json` and assets in the primary repository's draft release;
the standby repository may already have been deleted. The shared stdlib
consumer's source revision and checksum are in `deploy/ci-evidence-source.json`.
...
3. `source_run_id`: primary `Docker Branch Images` run id (path, head_branch=debug, head_sha exact), and it must equal the R0-1 workflow run bound into sealed local evidence. It must either succeed itself or have a verified standby receipt for a pure billing rejection; the primary failure is never relabeled as success.
...
Dispatch promotion through `gci dispatch promote-debug-image.yml --ref mine`
or the server's `gh workflow run` compatibility command with all four inputs.
The entry records those exact inputs before sending the primary request, then
replays them only for a confirmed billing rejection. Web or native `/usr/bin/gh`
dispatches cannot be replayed when their original inputs were never recorded.
Local verification uses `gci evidence` for both debug and promotion metadata,
and still re-hashes the sealed local evidence before production apply.
