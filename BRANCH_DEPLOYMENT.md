...
SharedChat no longer connects directly from Sub2API. Configure its complete
Router Egress Responses endpoint
(`http://172.17.0.1:13084/sharedchat/codex/responses`) and explicitly enable
`extra.openai_passthrough`; configuring only the Egress base prefix would make
the gateway append the wrong `/v1/responses` suffix. Configure the required
Codex `User-Agent`, `Originator`, `Version`, `OpenAI-Beta`, and `Accept` values
with account-level header overrides; those overrides are the final outbound
header values. Store a stable UUIDv4 in `extra.openai_device_id`; when the
client does not provide one, the gateway adds it as
`client_metadata.x-codex-installation-id`. Existing client metadata always
takes precedence.

For this endpoint, a downstream non-streaming Responses request is sent
upstream as a stream and aggregated back into one JSON response. This body-level
bridge cannot be expressed by header overrides. Streaming requests remain
streaming, and `/compact` remains a unary JSON request. The SharedChat endpoint
currently has no price route for `gpt-5.6-sol` with `reasoning.effort=max`; the
gateway therefore sends that exact combination as `xhigh`. Other hosts, models,
effort levels, `/compact`, and remote compaction requests are unchanged.

Router Egress owns SharedChat's fixed target, dedicated AppsGlobal Resin
identity, HTTP/1.1 transport, redirect refusal, single attempt, and exact quota
403-to-429 normalization. Sub2API retains account state, the Responses body
bridge, scheduling, usage, billing, and generic failure policy. Do not assign
the SharedChat Resin route as a Sub2API account proxy: Egress owns that hop.

Except for the explicitly preserved AnyRouter direct account, fixed OpenAI
API-key Providers use a base URL shaped as
`http://172.17.0.1:13084/profiles/<profile>/accounts/<account_id>` after the
Router manifest, CI, isolated matrix, and rollback gate pass. The profile must
allowlist Responses, compact, input_tokens, Chat, embeddings, alpha/search,
models, and billing as exact routes; `/v1/models` must keep the
`client_version` query and `If-None-Match` so the account page and Codex model
manifest continue to work.
