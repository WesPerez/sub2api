...
func TestOpenAIGatewayService_APIKeyPassthrough_PreservesBodyAndUsesResponsesEndpoint(t *testing.T) {
	...
	require.NotNil(t, upstream.lastReq)
	require.JSONEq(t, `{"model":"gpt-5.2","stream":false,"service_tier":"flex","max_output_tokens":128,"input":[{"type":"text","text":"hi"}],"reasoning":{"effort":"xhigh"}}`, string(upstream.lastBody))
