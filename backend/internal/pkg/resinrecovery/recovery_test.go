...
func TestRecoveryLimitedLeaseKeepsObservationsAndOtherAccountsAvailable(t *testing.T) {
	h := newRecoveryHarness(t, 1)
	h.acquireStatus = "recovery_limited"
	h.reportStatus = "recovery_limited"
	a := h.prepare(t, context.Background(), "limited", "https://example.com/responses")
	var retired atomic.Bool
	a.OnRetire(func(string) { retired.Store(true) })
	a.Complete(EmptyStream)
	if h.reportCount() != 1 || retired.Load() {
		t.Fatal("limited recovery must retain the lease and accept observations")
	}
	same := h.prepare(t, context.Background(), "limited", "https://example.com/responses")
	if same.Scope() != a.Scope() {
		t.Fatal("limited recovery replaced the unchanged lease scope")
	}
	same.Complete(EmptyStream)
	if h.reportCount() != 1 {
		t.Fatal("limited report did not retain per-lease backoff")
	}
	h.mu.Lock()
	h.acquireStatus, h.reportStatus = "available", "rotated"
	h.mu.Unlock()
	other := h.prepare(t, context.Background(), "other", "https://example.com/responses")
	other.Complete(EmptyStream)
	if h.reportCount() != 2 || h.acquires != 3 {
		t.Fatal("one account's recovery limit blocked another account")
	}
}
