...
func (m *Manager) Prepare(ctx context.Context, proxyURL string, target *url.URL) *Attempt {
	...
	// A recovery limit keeps the current lease valid. Continue observing it;
	// an account's cooldown must not back off the entire proxy endpoint.
	if err != nil || (result.Status != "available" && result.Status != "no_alternative" && result.Status != "recovery_limited") || !validLease(result.Lease, account) {
